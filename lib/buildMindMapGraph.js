...
function buildFinancialMindMapValue(quote) {
	...
	const metrics = [
		`營收 ${pickFirst([f.revenue, f.latestQuarterRevenue])}`,
		`EPS ${pickFirst([f.eps, f.latestQuarterEps])}`,
		`毛利率 ${pickFirst([f.grossMargin])}`,
		`淨利率 ${pickFirst([f.profitMargin])}`,
		`現金/負債 ${pickFirst([f.totalCash], "?")}/${pickFirst([f.debt], "?")}`,
		`FCF ${pickFirst([f.freeCashFlow])}`,
	];
	return metrics.join("｜");
}
...
export function buildMindMapGraph({ quote, language, t, displayValue }) {
	...
	const branches = [
		{ id: "moat", title: t.moat, value: quote.profile.moat || t.insufficientEvidence },
		{ id: "themes", title: t.themes, value: quote.profile.theme || t.insufficientEvidence },
		{
			id: "financials",
			title: t.financials,
		...
		},
		{
			id: "risks",
			title: t.risks,
			value: (quote.profile.risks || []).slice(0, 2).join("｜") || t.insufficientEvidence,
		},
	];
