...
export async function fetchAiTranslatedNews(symbol, news = []) {
	const normalized = (Array.isArray(news) ? news : []).slice(0, 5).map(normalizeNewsItem).filter((item) => item.title || item.text);
...
async function fetchAiTranslatedNewsDirect(symbol, news = []) {
	...
	try {
		...
		if (!response.ok) {
			console.error(`[AI News] OpenAI response not ok for ${symbol}:`, response.status);
			return [];
		}
		...
		console.error(`[AI News] Request failed for ${symbol}:`, error?.message || error);
		return [];
	}
}
