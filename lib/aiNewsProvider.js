...
import { AI_INPUT_LIMITS, getAiMaxOutputTokens, limitAiText } from "./aiPolicy.mjs";
...
function normalizeNewsItem(item) {
	return {
		...
		title: limitAiText(item?.title, AI_INPUT_LIMITS.newsTitle),
		text: limitAiText(item?.text, AI_INPUT_LIMITS.newsText),
	};
}
...
async function fetchAiTranslatedNewsDirect(symbol, news = []) {
	...
	try {
		...
		const payload = {
			model,
			max_output_tokens: getAiMaxOutputTokens(520),
			text: {
				format: {
					type: "json_schema",
					name: "translated_financial_news",
					strict: true,
					schema: {
						type: "object",
						properties: {
							items: {
								...
								items: {
									...
									required: ["index", "translatedTitle", "translatedText"],
