...
function generateDynamicRisks(snapshot) {
	...
	if (Number.isFinite(nextRevenueGrowth) && nextRevenueGrowth < 0) {
		risks.push(`需求循環風險：下一年度營收預估成長 ${snapshot.forecast?.nextAnnualRevenue?.growthDisplay || `${nextRevenueGrowth.toFixed(2)}%`}，需警惕景氣與市占波動。`);
	}
	if (Number.isFinite(nextEpsGrowth) && nextEpsGrowth < 0) {
	...
	if (Number.isFinite(insiderOwnership) && insiderOwnership < 1) {
		risks.push(`治理與激勵風險：內部人持股 ${metrics.sharesInsiders?.value || `${insiderOwnership.toFixed(2)}%`} 偏低，需追蹤管理層與股東利益一致性。`);
	}
