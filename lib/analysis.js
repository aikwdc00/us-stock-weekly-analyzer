...
export function getProfileForSymbol(symbol, snapshot) {
	...
	return {
		...
		competitors: Array.isArray(dynamicProfile.competitors) ? dynamicProfile.competitors : [],
		suppliers: Array.isArray(dynamicProfile.suppliers) ? dynamicProfile.suppliers : [],
		customers: Array.isArray(dynamicProfile.customers) ? dynamicProfile.customers : [],
		supplyChain: dynamicProfile.supplyChain || fallbackProfile.supplyChain,
		moat: dynamicProfile.moat || null,
		swot: deriveSourceDrivenSWOT(snapshot),
		risks: objectiveRisks || [],
		...
		valuationMethod: fallbackProfile.valuationMethod,
		valuationModels: fallbackProfile.valuationModels,
	};
}
...
function deriveSourceDrivenSWOT(snapshot) {
	...
	const sourceDriven = { s, w, o, t };

	const aiSwot = snapshot.aiSwot || null;
	if (!aiSwot) return s.length || w.length || o.length || t.length ? sourceDriven : null;
	...
	return merged.s.length || merged.w.length || merged.o.length || merged.t.length ? merged : null;
}
