...
function makePriceZones(current, low52, high52, valuation) {
	...
	// This is a transparent reference model, not a company-issued target price.
	...
	const rangeFloor = Number.isFinite(low52) && low52 > 0 && low52 < current ? low52 : current * 0.72;
	const rangeCeiling = Number.isFinite(high52) && high52 > current ? high52 : current * 1.12;
	const idealHigh = current * valuationDiscount;
	const idealLow = Math.min(idealHigh, Math.max(rangeFloor, idealHigh * 0.88));
	const buyHigh = current * 0.98;
	const buyLow = Math.min(idealHigh, buyHigh);
	const watchHigh = Math.max(rangeCeiling, current * 1.05);

	return {
		ideal: `${price(idealLow)} - ${price(idealHigh)}`,
		buy: `${price(buyLow)} - ${price(buyHigh)}`,
		watch: `${price(buyHigh)} - ${price(watchHigh)}`,
		isEstimate: true,
		basis: "客端估算：以行情 API 的現價、52 週高低點與估值分類（PE / Forward PE / PEG / FCF Yield）計算；不是公司揭露或分析師目標價。",
		method: "現價 × 估值折價係數，並以 52 週區間作為上下界參考",
	};
}
...
function growthDisplay(current, base) {
	if (!Number.isFinite(current) || !Number.isFinite(base) || base === 0) return null;
	return pct(((current - base) / Math.abs(base)) * 100);
}

function buildProfitForecast(quote, metrics, filingFinancials) {
	const forecast = quote.forecast || {};
	const annualRevenue = numericValue(forecast.annualRevenue?.current);
	const nextAnnualRevenue = numericValue(forecast.nextAnnualRevenue?.current);
	const annualFinancials = quote.financials?.annual || {};
	const filingAnnual = filingFinancials?.annual?.metrics || {};
	const annualGrossMargin =
		marginRatio(filingAnnual.grossMargin) ?? marginRatio(annualFinancials.raw?.grossMargin) ?? marginRatio(metrics.grossMargin?.number);
	const annualOperatingMargin =
		marginRatio(filingAnnual.operatingMargin) ??
		marginRatio(annualFinancials.raw?.operatingMargin) ??
		marginRatio(metrics.operatingMargin?.number);
	const annualGrossProfitActual = numericValue(filingAnnual.grossProfit) ?? numericValue(annualFinancials.raw?.grossProfit);
	const annualOperatingIncomeActual = numericValue(filingAnnual.operatingIncome) ?? numericValue(annualFinancials.raw?.operatingIncome);
	const previousAnnualRevenue = numericValue(forecast.annualRevenue?.last) ?? numericValue(annualFinancials.raw?.revenue);
	const annualGrossProfitBase =
		annualGrossProfitActual ??
		(Number.isFinite(previousAnnualRevenue) && Number.isFinite(annualGrossMargin) ? previousAnnualRevenue * annualGrossMargin : null);
	const annualOperatingIncomeBase =
		annualOperatingIncomeActual ??
		(Number.isFinite(previousAnnualRevenue) && Number.isFinite(annualOperatingMargin) ? previousAnnualRevenue * annualOperatingMargin : null);
	const annualGrossProfit = Number.isFinite(annualRevenue) && Number.isFinite(annualGrossMargin) ? annualRevenue * annualGrossMargin : null;
	const nextAnnualGrossProfit =
		Number.isFinite(nextAnnualRevenue) && Number.isFinite(annualGrossMargin) ? nextAnnualRevenue * annualGrossMargin : null;
	const annualOperatingIncome =
		Number.isFinite(annualRevenue) && Number.isFinite(annualOperatingMargin) ? annualRevenue * annualOperatingMargin : null;
	const nextAnnualOperatingIncome =
		Number.isFinite(nextAnnualRevenue) && Number.isFinite(annualOperatingMargin) ? nextAnnualRevenue * annualOperatingMargin : null;

	if (![annualGrossProfit, nextAnnualGrossProfit, annualOperatingIncome, nextAnnualOperatingIncome].some(Number.isFinite)) return null;

	const forecastSource = quote.fundamentalsSource === "StockAnalysis" ? "StockAnalysis Forecast" : quote.fundamentalsSource || "公開市場預估資料";
	const forecastSourceUrl = quote.fundamentalsSourceUrl ? quote.fundamentalsSourceUrl.replace("/statistics/", "/forecast/") : null;
	const marginSource = filingFinancials?.source || quote.fundamentalsSource || "公開財務資料";
	const marginPeriod = filingFinancials?.annual?.period || annualFinancials.year || "最新可取得年度";
	const marginReference = [
		Number.isFinite(annualGrossMargin) ? `毛利率 ${pct(annualGrossMargin * 100)}` : null,
		Number.isFinite(annualOperatingMargin) ? `營業利益率 ${pct(annualOperatingMargin * 100)}` : null,
	]
		.filter(Boolean)
		.join("、");

	return {
		annualGrossProfitDisplay: formatFinancialDisplay(annualGrossProfit),
		annualGrossProfitGrowthDisplay: growthDisplay(annualGrossProfit, annualGrossProfitBase),
		nextAnnualGrossProfitDisplay: formatFinancialDisplay(nextAnnualGrossProfit),
		nextAnnualGrossProfitGrowthDisplay: growthDisplay(nextAnnualGrossProfit, annualGrossProfit),
		annualOperatingIncomeDisplay: formatFinancialDisplay(annualOperatingIncome),
		annualOperatingIncomeGrowthDisplay: growthDisplay(annualOperatingIncome, annualOperatingIncomeBase),
		nextAnnualOperatingIncomeDisplay: formatFinancialDisplay(nextAnnualOperatingIncome),
		nextAnnualOperatingIncomeGrowthDisplay: growthDisplay(nextAnnualOperatingIncome, annualOperatingIncome),
		annualMarginReference: {
			grossMargin: Number.isFinite(annualGrossMargin) ? pct(annualGrossMargin * 100) : null,
			operatingMargin: Number.isFinite(annualOperatingMargin) ? pct(annualOperatingMargin * 100) : null,
		},
		assumptionNote: `預估營收取自 ${forecastSource}；${marginReference || "利潤率取自目前可取得的最新資料"}，基準期間為 ${marginPeriod}（${marginSource}）。公式：預估營收 × 最近可取得利潤率；這是計算值，不是公司或分析師直接發布的毛利／營業利益共識。`,
...
export function enrichQuote(quote, profile) {
	...
	const profitForecast = buildProfitForecast(quote, metrics, filingFinancials);
	...
	return {
		...
		fundamentals: {
			...
			estimatedAnnualGrossProfit: profitForecast?.annualGrossProfitDisplay || null,
			estimatedAnnualGrossProfitGrowth: profitForecast?.annualGrossProfitGrowthDisplay || null,
			estimatedNextAnnualGrossProfit: profitForecast?.nextAnnualGrossProfitDisplay || null,
			estimatedNextAnnualGrossProfitGrowth: profitForecast?.nextAnnualGrossProfitGrowthDisplay || null,
			estimatedAnnualOperatingIncome: profitForecast?.annualOperatingIncomeDisplay || null,
			estimatedAnnualOperatingIncomeGrowth: profitForecast?.annualOperatingIncomeGrowthDisplay || null,
			estimatedNextAnnualOperatingIncome: profitForecast?.nextAnnualOperatingIncomeDisplay || null,
			estimatedNextAnnualOperatingIncomeGrowth: profitForecast?.nextAnnualOperatingIncomeGrowthDisplay || null,
			...
			forecastAssumptionNote: profitForecast?.assumptionNote || null,
			forecastGrossMarginReference: profitForecast?.annualMarginReference?.grossMargin || null,
			forecastOperatingMarginReference: profitForecast?.annualMarginReference?.operatingMargin || null,
			...
			forecastSource: profitForecast?.source || null,
			forecastSourceUrl: profitForecast?.sourceUrl || null,
		...
		formatted: {
			...
			pe: formatMetricDisplay(metrics.pe, pe, "x"),
			forwardPe: formatMetricDisplay(metrics.peForward, forwardPe, "x"),
			ps: formatMetricDisplay(metrics.ps, ps, "x"),
			pfcf: formatMetricDisplay(metrics.pfcf, pfcf, "x"),
			fcfYield: cleanMetricDisplay(metrics.fcfYield?.value) || "N/A",
			pegRatio: cleanMetricDisplay(metrics.pegRatio?.value) || "N/A",
			marketCap: cleanMetricDisplay(metrics.marketcap?.value) || formatLargeNumber(quote.marketCap) || "N/A",
			...
			targetMeanPrice: cleanMetricDisplay(metrics.priceTarget?.value) || price(quote.targetMeanPrice),
			recommendation: cleanMetricDisplay(metrics.analystRatings?.value) || quote.averageAnalystRating || "N/A",
		},
	};
}
...
function cleanMetricDisplay(value) {
	const text = String(value || "").trim();
	return text && !/^(?:n\/a|na)$/i.test(text) ? text : null;
}

function formatMetricDisplay(metric, numericValue, suffix) {
	const display = cleanMetricDisplay(metric?.value);
	if (display) return `${display}${suffix}`;
	return Number.isFinite(numericValue) ? `${numericValue.toFixed(1)}${suffix}` : "N/A";
}
