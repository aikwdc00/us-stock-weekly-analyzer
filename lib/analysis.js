...
export function getProfileForSymbol(symbol, snapshot) {
	...
	const profileRisks = objectiveRisks?.length ? objectiveRisks : staticProfile?.risks?.length ? staticProfile.risks : fallbackProfile.risks;
...
function generateDynamicRisks(snapshot) {
	...
	if (risks.length === 0) {
		return ["需關注全球宏觀經濟波動、高利率環境對消費者支出或企業預算的影響。", "地緣政治與監管政策變化可能對供應鏈或跨國營運造成干擾。"];
	}
