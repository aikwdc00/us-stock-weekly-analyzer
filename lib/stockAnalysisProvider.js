...
function metricFromHtml(html, id) {
	const regex = new RegExp(`id:"${id}"[^}]*?title:"([^"]+)"[^}]*?value:"([^"]*)"[^}]*?(?:hover:"([^"]*)")?`);
...
function numberFromQuoteBlock(html, key) {
	const quoteBlock = html.match(/quote:\{([^}]*)\}/)?.[1];
	const match = quoteBlock?.match(new RegExp(`(?:^|,)${key}:(-?\\d+(?:\\.\\d+)?)`));
...
function parseProfileInfo(html) {
	...
	const infoTableMatch = html.match(/infoTable:\[([\s\S]*?)\]/);
	const analystIntroMatch = html.match(/analystIntro:"([\s\S]*?)"/);
	const financialIntroMatch = html.match(/financialIntro:"([\s\S]*?)"/);

	let description = descriptionMatch ? cleanHtmlText(descriptionMatch[1]) : null;
	let analystIntro = analystIntroMatch ? cleanHtmlText(analystIntroMatch[1]) : null;
	let financialIntro = financialIntroMatch ? cleanHtmlText(financialIntroMatch[1]) : null;
	let industry = null;
	let sector = null;

	if (infoTableMatch) {
		const tableText = infoTableMatch[1];
		const industryMatch = tableText.match(/\{t:"Industry",v:"([^"]+)"/);
		const sectorMatch = tableText.match(/\{t:"Sector",v:"([^"]+)"/);
		if (industryMatch) industry = cleanHtmlText(industryMatch[1]);
		if (sectorMatch) sector = cleanHtmlText(sectorMatch[1]);
	}

	return { description, industry, sector, analystIntro, financialIntro };
}
...
function growthRate(current, base) {
	return Number.isFinite(current) && Number.isFinite(base) && base !== 0 ? ((current - base) / Math.abs(base)) * 100 : null;
}

function buildProjectedProfitForecast(income, forecast, latestAnnual) {
	if (!income || !forecast) return null;

	const annualRevenueActual = rowAt(income, "revenue", latestAnnual);
	const annualGrossProfitActual = rowAt(income, "grossProfit", latestAnnual);
	const annualOperatingIncomeActual = rowAt(income, "operatingIncome", latestAnnual);
	const annualGrossMargin = rowAt(income, "grossMargin", latestAnnual);
	const annualOperatingMargin = rowAt(income, "operatingMargin", latestAnnual);

	const annualRevenueEstimate = forecast.annualRevenue?.current ?? null;
	const nextAnnualRevenueEstimate = forecast.nextAnnualRevenue?.current ?? null;

	const annualGrossProfitEstimate =
		Number.isFinite(annualRevenueEstimate) && Number.isFinite(annualGrossMargin) ? annualRevenueEstimate * annualGrossMargin : null;
	const nextAnnualGrossProfitEstimate =
		Number.isFinite(nextAnnualRevenueEstimate) && Number.isFinite(annualGrossMargin) ? nextAnnualRevenueEstimate * annualGrossMargin : null;
	const annualOperatingIncomeEstimate =
		Number.isFinite(annualRevenueEstimate) && Number.isFinite(annualOperatingMargin) ? annualRevenueEstimate * annualOperatingMargin : null;
	const nextAnnualOperatingIncomeEstimate =
		Number.isFinite(nextAnnualRevenueEstimate) && Number.isFinite(annualOperatingMargin)
			? nextAnnualRevenueEstimate * annualOperatingMargin
			: null;

	return {
		assumptionNote:
			"StockAnalysis forecast 頁面未直接提供毛利與營業利益共識；此處以最近年度毛利率與營業利益率，套用到本年度與下一年度預估營收進行推估。",
		annualMarginReference: {
			grossMargin: formatRatio(annualGrossMargin),
			operatingMargin: formatRatio(annualOperatingMargin),
		},
		annualGrossProfit: annualGrossProfitEstimate,
		annualGrossProfitDisplay: formatFinancialValue(annualGrossProfitEstimate),
		annualGrossProfitGrowth: growthRate(annualGrossProfitEstimate, annualGrossProfitActual),
		annualGrossProfitGrowthDisplay: formatPercent(growthRate(annualGrossProfitEstimate, annualGrossProfitActual)),
		nextAnnualGrossProfit: nextAnnualGrossProfitEstimate,
		nextAnnualGrossProfitDisplay: formatFinancialValue(nextAnnualGrossProfitEstimate),
		nextAnnualGrossProfitGrowth: growthRate(nextAnnualGrossProfitEstimate, annualGrossProfitEstimate),
		nextAnnualGrossProfitGrowthDisplay: formatPercent(growthRate(nextAnnualGrossProfitEstimate, annualGrossProfitEstimate)),
		annualOperatingIncome: annualOperatingIncomeEstimate,
		annualOperatingIncomeDisplay: formatFinancialValue(annualOperatingIncomeEstimate),
		annualOperatingIncomeGrowth: growthRate(annualOperatingIncomeEstimate, annualOperatingIncomeActual),
		annualOperatingIncomeGrowthDisplay: formatPercent(growthRate(annualOperatingIncomeEstimate, annualOperatingIncomeActual)),
		nextAnnualOperatingIncome: nextAnnualOperatingIncomeEstimate,
		nextAnnualOperatingIncomeDisplay: formatFinancialValue(nextAnnualOperatingIncomeEstimate),
		nextAnnualOperatingIncomeGrowth: growthRate(nextAnnualOperatingIncomeEstimate, annualOperatingIncomeEstimate),
		nextAnnualOperatingIncomeGrowthDisplay: formatPercent(growthRate(nextAnnualOperatingIncomeEstimate, annualOperatingIncomeEstimate)),
		annualActualRevenue: annualRevenueActual,
		annualActualGrossProfit: annualGrossProfitActual,
		annualActualOperatingIncome: annualOperatingIncomeActual,
	};
}
...
function buildFinancialSummary(income, cashFlow, forecast) {
	...
	const rndGrowth = Number.isFinite(rnd) && Number.isFinite(previousRnd) && previousRnd !== 0 ? (rnd - previousRnd) / Math.abs(previousRnd) : null;
	const projectedProfitForecast = buildProjectedProfitForecast(income, forecast, latestAnnual);
	...
	return {
		ttm: {
			...
			raw: {
				revenue: rowAt(income, "revenue", ttm),
				costOfRevenue: rowAt(income, "costOfRevenue", ttm),
				grossProfit: rowAt(income, "grossProfit", ttm),
				operatingIncome: rowAt(income, "operatingIncome", ttm),
				netIncome: rowAt(income, "netIncome", ttm),
				grossMargin: rowAt(income, "grossMargin", ttm),
				operatingMargin: rowAt(income, "operatingMargin", ttm),
				profitMargin: rowAt(income, "profitMargin", ttm),
			},
		},
		annual: {
			...
			raw: {
				...
				costOfRevenue: annualCost,
				grossProfit: rowAt(income, "grossProfit", latestAnnual),
				operatingExpenses: operatingExpensesAt(income, latestAnnual),
				operatingIncome: rowAt(income, "operatingIncome", latestAnnual),
				netIncome: rowAt(income, "netIncome", latestAnnual),
				grossMargin: rowAt(income, "grossMargin", latestAnnual),
				operatingMargin: rowAt(income, "operatingMargin", latestAnnual),
				profitMargin: rowAt(income, "profitMargin", latestAnnual),
			},
		},
		forecast: projectedProfitForecast,
...
function parseLatestQuarter(html) {
	const match = html.match(/quarterly:\{eps:\[([\s\S]*?)\],dates:\[([\s\S]*?)\],revenue:\[([\s\S]*?)\][\s\S]*?lastDate:(\d+)/);
	if (!match) return null;
...
export async function fetchStockAnalysisSnapshot(symbol) {
	...
	try {
		...
		const profile = parseProfileInfo(overviewHtml);
		const forecast = parseForecastSnapshot(forecastHtml);
		...
		return {
			...
			metrics,
			forecast,
			financials: buildFinancialSummary(income, cashFlow, forecast),
