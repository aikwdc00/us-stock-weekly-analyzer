...
export function getClientKey(request, scope) {
	const trustedProxy = String(process.env.TRUST_PROXY_HEADERS || "").toLowerCase() === "true";
	const realIp = trustedProxy ? request.headers.get("x-real-ip")?.trim() : "";
	const forwarded = trustedProxy ? request.headers.get("x-forwarded-for")?.split(",")[0]?.trim() : "";
	const fingerprint = [request.headers.get("user-agent") || "unknown", request.headers.get("accept-language") || "unknown"].join("|");
	const address = (realIp || forwarded || `anonymous:${fingerprint}`).slice(0, 128);
