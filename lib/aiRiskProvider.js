import { createInsightDigest, getInsightWithGuard } from "./aiInsightCache";
import { getAiMaxOutputTokens, limitAiText } from "./aiPolicy.mjs";
...
function compactSnapshot(snapshot) {
	...
	return {
		...
		news: (snapshot?.news || []).slice(0, 3).map((item) => ({
			source: item.source,
			title: limitAiText(item.title, 320),
		})),
	};
}

function buildContextDigest(snapshot, finnhub, profile) {
	return createInsightDigest({
		snapshot: compactSnapshot(snapshot),
		recommendations: finnhub?.recommendations || {},
		theme: limitAiText(profile?.theme, 500),
	});
}
...
export async function fetchAiRiskInsights(symbol, snapshot, finnhub = {}, profile = {}) {
	return getInsightWithGuard({
		symbol,
		type: "risk",
		contextDigest: buildContextDigest(snapshot, finnhub, profile),
...
async function fetchAiRiskInsightsDirect(symbol, snapshot, finnhub = {}, profile = {}) {
	...
	try {
		...
		const payload = {
			model,
			max_output_tokens: getAiMaxOutputTokens(420),
			text: {
				format: {
					type: "json_schema",
					name: "equity_risks",
					...
					schema: {
						type: "object",
						properties: { risks: { type: "array", items: { type: "string" }, maxItems: 6 } },
						required: ["risks"],
...
async function fetchAiSwotInsightsDirect(symbol, snapshot, finnhub = {}, profile = {}) {
	...
	try {
		...
		const payload = {
			model,
			max_output_tokens: getAiMaxOutputTokens(620),
			text: {
				format: {
					type: "json_schema",
					name: "equity_swot",
					...
					schema: {
						type: "object",
						properties: {
							s: { type: "array", items: { type: "string" }, maxItems: 5 },
							w: { type: "array", items: { type: "string" }, maxItems: 5 },
							o: { type: "array", items: { type: "string" }, maxItems: 5 },
							t: { type: "array", items: { type: "string" }, maxItems: 5 },
						},
						required: ["s", "w", "o", "t"],
						additionalProperties: false,
					},
			...
			input: [
				...
				{
					role: "user",
					content: `請針對 ${symbol} 產出 SWOT，只回傳符合 schema 的 JSON。\n要求：\n1) 每個維度 3-5 點。\n2) 每點必須是可驗證、可追蹤的客觀敘述。\n3) 優先使用估值、成長、獲利、現金流、負債、分析師共識、近期新聞。\n4) 禁止空泛口號。\n\n公司題材：${limitAiText(profile?.theme, 500) || "N/A"}\n資料摘要：${JSON.stringify(compactSnapshot(snapshot))}\n分析師補充：${JSON.stringify(finnhub?.recommendations || {})}`,
				},
			],
		};
