...
export async function GET(request) {
  ...
  try {
    const universe = (await fetchMarketUniverse()).map((row, index) => ({ ...row, rank: index + 1 }));
    const snapshots = await Promise.all(
      universe.map((row) => fetchStockAnalysisSnapshot(row.symbol).catch(() => ({ metrics: {} })))
    );
      ...
      groups: [
        {
          ...
          criteria:
            "即時候選池中，優先篩選大型企業、資料完整、FCF Yield 為正、Forward PE 較可控、毛利率/淨利率較佳且仍有營收成長者。",
          criteriaEn:
        ...
        },
        {
          ...
          criteria:
            "即時候選池中，優先篩選預估營收成長、下一年度成長、最新季度成長、毛利率與技術面動能較強者，同時扣除估值過熱風險。",
          criteriaEn:
