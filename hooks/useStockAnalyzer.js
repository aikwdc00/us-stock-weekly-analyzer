...
export function useStockAnalyzer() {
  ...
  const recommendationsState = useRecommendations(watchlistState.watchlist, { onError: reportError });
  ...
  const addSymbol = useCallback(
    async (rawSymbol) => {
      ...
      await Promise.all([
        quotesState.refreshQuotes(next),
        recommendationsState.refreshRecommendations(next)
      ]);
    },
  ...
  const coverageStats = useMemo(() => {
    ...
    const avgMove =
      quotesState.quotes.reduce((sum, quote) => sum + (safeNumber(quote.changePercent) ?? 0), 0) / Math.max(loaded, 1);
