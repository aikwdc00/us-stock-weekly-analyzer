    ...
    <div className="reportTabPanel">
      <section className="analysisGrid">
        <AnalysisBlock 
          ...
          items={[
            translateTerm(quote.profile.sector, language), 
            translateTerm(quote.profile.industry, language)
          ].filter(Boolean)} 
        />
        <AnalysisBlock 
          title={language === "zh" ? "競爭格局 (清單)" : "Competitors (Watchlist)"} 
          items={quote.profile.competitors} 
        />
        <div className="analysisCard">
          <div className="sectionTitle">
            <h3>{language === "zh" ? "公司摘要" : "Company Summary"}</h3>
            {quote.fundamentals.sourceUrl && (
              <a href={quote.fundamentals.sourceUrl.replace("/statistics/", "/")} target="_blank" rel="noreferrer" className="sourceIconLink">
                <ExternalLink size={14} />
              </a>
            )}
          </div>
          <p className="summaryText">
            {quote.profile.description ? `${quote.profile.description.slice(0, 300)}...` : (language === "zh" ? "暫無描述" : "No description")}
          </p>
        </div>
      </section>
        ...
        <div className="sectionTitle">
          ...
          <div className="tooltipAnchor">
            ...
            <div className="tooltipBubble">
              {t.peerSource}: Yahoo Finance Recommendations<br />
